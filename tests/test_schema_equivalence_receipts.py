...
def test_no_forbidden_imports_decoder_boundary_and_pythonhashseed_stability():
    text = open("src/qec/analysis/schema_equivalence_receipts.py", encoding="utf-8").read()
    ...
    a = subprocess.check_output(["python", "-c", cmd], env=env, text=True).strip()
    b = subprocess.check_output(["python", "-c", cmd], env=env, text=True).strip()
    assert a == b
