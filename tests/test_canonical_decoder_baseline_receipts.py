...
def _parts():
    ...
    return identity, source_boundary, replay_boundary, equivalence_policy, immutability_boundary, receipt
...
def test_happy_path_validates_hash_and_frozen_artifacts():
    identity, source_boundary, replay_boundary, equivalence_policy, immutability_boundary, receipt = _parts()
...
def test_canonical_json_and_hash_determinism_for_ordering_and_hash_seed_independence():
    ...
    third = cdb.build_canonical_decoder_source_boundary(list({ordered[0].path: ordered[0], ordered[1].path: ordered[1]}.values()))

    assert first.source_files == ordered
    assert first.source_tree_hash == second.source_tree_hash == third.source_tree_hash
    assert first.canonical_decoder_source_boundary_hash == second.canonical_decoder_source_boundary_hash == third.canonical_decoder_source_boundary_hash
...
def test_self_hash_exclusion_and_stale_hash_failures():
    ...
    assert cdb._hash_payload(aggregate_payload) == receipt.canonical_decoder_baseline_receipt_hash
    assert "canonical_decoder_source_boundary_hash" not in cdb._source_boundary_payload(source_boundary)
    assert "canonical_decoder_replay_corpus_boundary_hash" not in cdb._replay_corpus_boundary_payload(replay_boundary)
    assert "canonical_decoder_equivalence_policy_hash" not in cdb._equivalence_policy_payload(equivalence_policy)
    assert "canonical_decoder_immutability_boundary_hash" not in cdb._immutability_boundary_payload(immutability_boundary)


def test_child_before_aggregate_validation_rejects_corrupt_child():
    identity, source_boundary, replay_boundary, equivalence_policy, immutability_boundary, _ = _parts()
    ...
    object.__setattr__(corrupt_boundary, "canonical_decoder_source_boundary_hash", HEX_B)
    ...
    object.__setattr__(parent, "canonical_decoder_baseline_receipt_hash", cdb._hash_payload(values))

    _expect_error("INVALID_HASH", cdb.validate_canonical_decoder_baseline_receipt, parent)
...
def test_identity_validation(kwargs, error):
    _expect_error(error, cdb.build_canonical_decoder_identity, "canonical decoder baseline", **kwargs)


@pytest.mark.parametrize("bad_hash", ["A" * 64, "a" * 63, "g" * 64])
def test_source_file_rejects_invalid_hashes(bad_hash):
    _expect_error("INVALID_HASH", cdb.build_canonical_decoder_source_file, "src/qec/decoder/a.py", bad_hash)


@pytest.mark.parametrize("path", ["/src/qec/decoder/a.py", "src/qec/decoder/../a.py", "src/qec/other/a.py"])
def test_source_file_rejects_invalid_paths(path):
    _expect_error("INVALID_INPUT", cdb.build_canonical_decoder_source_file, path, HEX_A)
...
def test_source_boundary_validation_failures_and_recomputations():
    ...
    _expect_error("INVALID_INPUT", cdb.build_canonical_decoder_source_boundary, (sf, sf))
    _expect_error("INVALID_DECODER_BASELINE", cdb.build_canonical_decoder_source_boundary, (sf,), runtime_decoder_execution_allowed=True)
    _expect_error("INVALID_DECODER_BASELINE", cdb.build_canonical_decoder_source_boundary, (sf,), decoder_import_allowed=True)
    _expect_error("INVALID_DECODER_BASELINE", cdb.build_canonical_decoder_source_boundary, (sf,), mutation_allowed=True)

    boundary = cdb.build_canonical_decoder_source_boundary((sf,))
    _expect_error("INVALID_INPUT", replace, boundary, source_file_count=2, canonical_decoder_source_boundary_hash=boundary.canonical_decoder_source_boundary_hash)
...
def test_replay_corpus_boundary_validation_failures():
    for field in ("corpus_hash", "input_schema_hash", "output_schema_hash"):
        kwargs = {"corpus_name": "declared corpus", "corpus_version": "v1", "corpus_hash": HEX_A, "input_schema_hash": HEX_B, "output_schema_hash": HEX_C, field: "A" * 64}
        _expect_error("INVALID_HASH", cdb.build_canonical_decoder_replay_corpus_boundary, **kwargs)
    _expect_error("INVALID_INPUT", cdb.build_canonical_decoder_replay_corpus_boundary, "declared corpus", "v1", HEX_A, HEX_B, HEX_C, corpus_mode="CUSTOM")
    _expect_error("INVALID_DECODER_BASELINE", cdb.build_canonical_decoder_replay_corpus_boundary, "declared corpus", "v1", HEX_A, HEX_B, HEX_C, runtime_decoder_execution_allowed=True)
    _expect_error("INVALID_DECODER_BASELINE", cdb.build_canonical_decoder_replay_corpus_boundary, "declared corpus", "v1", HEX_A, HEX_B, HEX_C, candidate_replay_required_before_promotion=False)
    _expect_error("INVALID_INPUT", cdb.build_canonical_decoder_replay_corpus_boundary, "declared corpus", "v1", HEX_A, HEX_B, HEX_C, syndrome_ordering_policy="")
    _expect_error("INVALID_INPUT", cdb.build_canonical_decoder_replay_corpus_boundary, "declared corpus", "v1", HEX_A, HEX_B, HEX_C, syndrome_ordering_policy="RANDOM_ORDER")


def test_equivalence_policy_validation_failures():
    cdb.validate_canonical_decoder_equivalence_policy(cdb.build_canonical_decoder_equivalence_policy())
    _expect_error("INVALID_INPUT", cdb.build_canonical_decoder_equivalence_policy, equivalence_mode="APPROXIMATE_OUTPUT_MATCH")
    _expect_error("INVALID_INPUT", cdb.build_canonical_decoder_equivalence_policy, equivalence_mode="PROBABILISTIC_OUTPUT_MATCH")
    _expect_error("INVALID_INPUT", cdb.build_canonical_decoder_equivalence_policy, precision_policy="hidden precision drift")
    _expect_error("INVALID_INPUT", cdb.build_canonical_decoder_equivalence_policy, approximation_policy="undeclared approximation policy")
    _expect_error("INVALID_DECODER_BASELINE", cdb.build_canonical_decoder_equivalence_policy, benchmark_claims_allowed=True)
    _expect_error("INVALID_DECODER_BASELINE", cdb.build_canonical_decoder_equivalence_policy, hardware_authority_allowed=True)
    _expect_error("INVALID_DECODER_BASELINE", cdb.build_canonical_decoder_equivalence_policy, probabilistic_promotion_allowed=True)
    _expect_error("INVALID_DECODER_BASELINE", cdb.build_canonical_decoder_equivalence_policy, silent_replacement_allowed=True)


def test_immutability_boundary_validation_failures():
    _expect_error("INVALID_INPUT", cdb.build_canonical_decoder_immutability_boundary, ())
    _expect_error("INVALID_INPUT", cdb.build_canonical_decoder_immutability_boundary, ("src/qec/decoder/a.py", "src/qec/decoder/a.py"))
    _expect_error("INVALID_INPUT", cdb.build_canonical_decoder_immutability_boundary, ("src/qec/other/a.py",))
    _expect_error("INVALID_DECODER_BASELINE", cdb.build_canonical_decoder_immutability_boundary, ("src/qec/decoder/a.py",), mutation_allowed=True)
    _expect_error("INVALID_DECODER_BASELINE", cdb.build_canonical_decoder_immutability_boundary, ("src/qec/decoder/a.py",), silent_replacement_allowed=True)
    _expect_error("INVALID_DECODER_BASELINE", cdb.build_canonical_decoder_immutability_boundary, ("src/qec/decoder/a.py",), candidate_implementation_allowed=True)
    _expect_error("INVALID_DECODER_BASELINE", cdb.build_canonical_decoder_immutability_boundary, ("src/qec/decoder/a.py",), runtime_promotion_allowed=True)
    _expect_error("INVALID_DECODER_BASELINE", cdb.build_canonical_decoder_immutability_boundary, ("src/qec/decoder/a.py",), rollback_required_for_future_promotion=False)
...
def test_boundary_no_decoder_import_heavy_imports_network_benchmark_or_runtime_decoder_execution():
    ...
    assert not any(name == "qec.decoder" or name.startswith("qec.decoder.") for name in module_imports)
    assert not any(name == "qec.decoder" or name.startswith("qec.decoder.") for name in test_imports)
    assert module_imports.isdisjoint({"numpy", "scipy", "qldpc", "stim", "pymatching", "qiskit", "requests", "urllib", "socket", "time", "datetime", "random"})
...
"""
    ...
    for seed in ("0", "1"):
        ...
        outputs.append(subprocess.check_output([sys.executable, "-c", script], cwd=Path(__file__).parents[1], env=env, text=True).strip())
