...
def test_idempotent_and_immutable_and_pyhashseed_stable(monkeypatch):
    ...
    a = _receipt(m, p).human_review_boundary_receipt_hash
    monkeypatch.setenv("PYTHONHASHSEED", "0")
    b = _receipt(m, p).human_review_boundary_receipt_hash
    assert a == b
...
def test_no_forbidden_imports_and_decoder_boundary():
    ...
    for token in ("requests", "urllib", "selenium", "playwright", "pandas", "polars", "openai", "anthropic", "transformers", "torch", "tensorflow", "qiskit", "qutip", "subprocess", "importlib", "os.system"):
