...
CLASSIFICATION_NO_OP = "no_op"
CLASSIFICATION_PARTIAL_FRAGMENT_REPAIR = "partial_fragment_repair"
CLASSIFICATION_UNRECOVERABLE_CORRUPTION = "unrecoverable_corruption"
...
def _score_replay_identity(
    ...
    repaired_payload = {
        ...
        "preserved_theme_hashes": source_artifact.preserved_theme_hashes,
        "compression_chain_head": repaired_chain_head,
        "replay_identity_hash": source_artifact.replay_identity_hash,
    }
...
def _expected_classification(recovery_artifact: FragmentationRecoveryArtifact) -> str:
    ...
    if len(report.corrupted_theme_indices) > 0 and len(report.missing_theme_indices) == report.source_record_count:
        return CLASSIFICATION_UNRECOVERABLE_CORRUPTION
    if len(report.missing_theme_indices) > 0 or len(report.disordered_theme_indices) > 0:
        return CLASSIFICATION_PARTIAL_FRAGMENT_REPAIR
    return CLASSIFICATION_NO_OP
...
def _score_recovery_correctness(recovery_artifact: FragmentationRecoveryArtifact) -> float:
    ...
    confidence_alignment = 1.0 if expected != CLASSIFICATION_UNRECOVERABLE_CORRUPTION else 0.0
...
def run_memory_fidelity_benchmark(
    ...
    scores = {
        ...
        "replay_fidelity_score": _score_replay_identity(
            source_artifact,
            repaired_records,
            recovery_artifact.repaired_chain_head,
        ),
        ...
        "noop_path_fidelity_score": _score_path_fidelity(recovery_artifact, target=CLASSIFICATION_NO_OP),
        "partial_repair_fidelity_score": _score_path_fidelity(
            recovery_artifact,
            target=CLASSIFICATION_PARTIAL_FRAGMENT_REPAIR,
        ),
        "unrecoverable_corruption_classification_fidelity_score": _score_path_fidelity(
            recovery_artifact,
            target=CLASSIFICATION_UNRECOVERABLE_CORRUPTION,
        ),
    }
    ...
    snapshot = replace(snapshot, snapshot_hash=snapshot.stable_hash())
    ...
    return replace(result, benchmark_hash=result.stable_hash())
