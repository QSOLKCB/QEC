...
def run_parity_coherence_analysis(
    ...
    coherence_length = _longest_nonzero_run(nonzero_signs)
    parity_jump_detected = _has_persistent_jump(nonzero_signs, min_run=2)

    if nonzero_count > 0 and global_probe_score >= 0.6:
    ...
    parity_stability_score = _clamp01(
        0.5 * global_probe_score
        + 0.3 * coherence_ratio
        + 0.2 * (1.0 - probe_disagreement)
    )
...
def _has_persistent_jump(signs: List[int], min_run: int) -> bool:
    ...
    for i in range(len(runs) - 1):
        left_sign, left_len = runs[i]
        right_sign, right_len = runs[i + 1]
