...
from qec.analysis.memory_fidelity_benchmark import (
    ...
    CLASSIFICATION_PARTIAL_FRAGMENT_REPAIR,
    CLASSIFICATION_UNRECOVERABLE_CORRUPTION,
...
def test_perfect_chain_fidelity_is_one() -> None:
    ...
    assert result.snapshot.classification == CLASSIFICATION_NO_OP
...
def test_partial_fracture_fidelity_is_less_than_one() -> None:
    ...
    result = run_memory_fidelity_benchmark(compressed, recovery)
    assert result.snapshot.classification == CLASSIFICATION_PARTIAL_FRAGMENT_REPAIR
    assert result.snapshot.overall_fidelity_score < 1.0
...
def test_stable_hash_and_receipt_determinism() -> None:
    ...
    assert benchmark.stable_hash() == benchmark.result_hash
...
def test_invalid_source_chain_head_fails_fast() -> None:
    compressed = _compressed_artifact()
    invalid = type(compressed)(
        ...
        compression_chain_head=compressed.records[0].source_replay_identity_hash,
    ...
    with pytest.raises(ValueError, match="compression_chain_head must match last replay identity"):
        run_memory_fidelity_benchmark(invalid, recovery)


def test_non_contiguous_theme_indices_fail_fast() -> None:
    compressed = _compressed_artifact()
    broken = type(compressed.records[1])(
    ...
    mutated_records = (compressed.records[0], broken) + compressed.records[2:]
    invalid = type(compressed)(
        schema_version=compressed.schema_version,
        source_artifact_hash=compressed.source_artifact_hash,
        source_theme_count=compressed.source_theme_count,
    ...
    with pytest.raises(ValueError, match="contiguous in theme_index"):
        run_memory_fidelity_benchmark(invalid, recovery)


def test_invalid_repaired_chain_head_fails_fast() -> None:
    compressed = _compressed_artifact()
    recovery = recover_fragmented_compression_chain(
        compressed,
        observed_records=tuple(compressed.records),
    ...
    invalid = type(recovery)(
        ...
        repaired_chain_head=recovery.repaired_records[0].source_replay_identity_hash,
        repaired_records=recovery.repaired_records,
        preserved_theme_hashes=recovery.preserved_theme_hashes,
    ...
    with pytest.raises(ValueError, match="repaired_chain_head must match final repaired record"):
        run_memory_fidelity_benchmark(compressed, invalid)


def test_mismatched_preserved_theme_hashes_fail_fast() -> None:
    compressed = _compressed_artifact()
    recovery = recover_fragmented_compression_chain(
        compressed,
        observed_records=tuple(compressed.records),
...
def test_replay_fidelity_degrades_when_chain_head_mismatch_path_is_used() -> None:
    compressed = _compressed_artifact()
    recovery = recover_fragmented_compression_chain(
        compressed,
        observed_records=tuple(compressed.records),
    ...
    mutated_record = type(recovery.repaired_records[0])(
        theme_id=recovery.repaired_records[0].theme_id,
        theme_index=recovery.repaired_records[0].theme_index,
        source_theme_hash=recovery.repaired_records[1].source_theme_hash,
        source_replay_identity_hash=recovery.repaired_records[0].source_replay_identity_hash,
        source_parent_theme_hash=recovery.repaired_records[0].source_parent_theme_hash,
        signature_ref=recovery.repaired_records[0].signature_ref,
        reason_ref=recovery.repaired_records[0].reason_ref,
        episode_hashes_ref=recovery.repaired_records[0].episode_hashes_ref,
        compression_record_hash=recovery.repaired_records[0].compression_record_hash,
    )
    degraded_records = (mutated_record,) + recovery.repaired_records[1:]
    degraded = type(recovery)(
        schema_version=recovery.schema_version,
        source_compression_hash=recovery.source_compression_hash,
        source_replay_identity_hash=recovery.source_replay_identity_hash,
        repaired_chain_head=recovery.repaired_chain_head,
        repaired_records=degraded_records,
        preserved_theme_hashes=recovery.preserved_theme_hashes,
        continuity_score=recovery.continuity_score,
        recovery_confidence=recovery.recovery_confidence,
        fracture_severity=recovery.fracture_severity,
    ...
    result = run_memory_fidelity_benchmark(compressed, degraded)
    assert result.snapshot.replay_fidelity_score == 0.0


def test_classification_constants_are_used() -> None:
    compressed = _compressed_artifact()
    no_op = recover_fragmented_compression_chain(
        compressed,
        observed_records=tuple(compressed.records),
        enable_fragmentation_recovery=True,
    )
    partial = recover_fragmented_compression_chain(
        compressed,
        observed_records=(compressed.records[2], compressed.records[0]),
        enable_fragmentation_recovery=True,
    )
    ...
    unrecoverable = recover_fragmented_compression_chain(
        compressed,
        observed_records=(wrong,),
        enable_fragmentation_recovery=True,
    )
    assert run_memory_fidelity_benchmark(compressed, no_op).snapshot.classification == CLASSIFICATION_NO_OP
    assert run_memory_fidelity_benchmark(compressed, partial).snapshot.classification == CLASSIFICATION_PARTIAL_FRAGMENT_REPAIR
    assert (
        run_memory_fidelity_benchmark(compressed, unrecoverable).snapshot.classification
        == CLASSIFICATION_UNRECOVERABLE_CORRUPTION
    )
