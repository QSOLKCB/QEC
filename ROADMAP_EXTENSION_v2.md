...
Rydberg atom platforms provide a physical context in which multi-level quantum states are accessible through principal quantum number excitation. Research on quantum network tomography of small Rydberg arrays (Mukherjee et al., Phys. Rev. A 111, 043319, 2025) demonstrates that ML-extracted interaction Hamiltonians from Rydberg array transport experiments can identify node count, location, and effective decoherence operators. This is a source-grounded signal that qudit-like multi-level Rydberg states are experimentally accessible. It is not a proof that qudit codes outperform qubit codes. The ML-extracted Hamiltonian is a model output, not a ground truth. Any qudit code claim that references Rydberg array experiments must declare: system size, training corpus size, simulation-to-experiment transfer policy, and UNREVIEWED_PREPRINT status where applicable. The steep training data requirement identified in that work (best suited for networks of just a few sites) must be declared as a system-size limitation in any QuditBenchmarkBoundaryReceipt that references Rydberg array evidence.

GF(d) encoding policy realism requires explicit declaration of: field characteristic (prime p for GF(p) or prime power q for GF(q)), element representation (polynomial basis, normal basis, or integer residue), arithmetic implementation (lookup table, Berlekamp-Massey, or direct polynomial), and canonicalization policy (lexicographic ordering of generator matrix rows). No GF(d) encoding is accepted without these declarations. Silent GF(2) fallback for d=2 is permitted only when explicitly declared.
...
A Rydberg array experiment is a source-bound signal.
An ML-extracted Hamiltonian is a model output.
Neither is a qudit code proof.

The GF(d) encoding policy is declared.
The system-size limitation is declared.
...
+ same Galois field GF(d) encoding policy
  (field characteristic, element representation, arithmetic implementation,
...
**Acceptance Gates**
- pytest: QuditDimensionPolicyManifest declares local dimension d, field GF(d), element representation, arithmetic implementation, and canonicalization policy
- pytest: QuditDimensionPolicyManifest declares GF(2) fallback explicitly when d=2
...
- pytest: QuditBenchmarkBoundaryReceipt declares system-size limitation when referencing Rydberg array evidence
- pytest: any Rydberg array reference must declare simulation-to-experiment transfer policy and UNREVIEWED_PREPRINT status where applicable
- pytest: ML-extracted Hamiltonians from Rydberg array tomography are labelled model_output=true, not ground_truth
...
- dependency boundary: Rydberg array experiments are source-bound signals, not QEC authority
...
- no undeclared GF(d) element representation
- no undeclared GF(d) arithmetic implementation
...
- no Rydberg array experiment cited as qudit code proof
- no ML-extracted Hamiltonian treated as ground truth
- no system-size limitation omitted from benchmark receipts referencing small-array evidence
...
- source signals: Mukherjee et al., Phys. Rev. A 111, 043319 (2025) — Rydberg array tomography (adapter signal only; adapter_only=true)
- does not modify: src/qec/decoder/

**Relationship to Existing Roadmap**
v167.x extends the decoder governance discipline of v166.x into the high-dimensional stabilizer space. It does not reimplement the v1.5/v1.6/v91.x/v134.x qudit engines. It wraps their behavioral contracts as receipt-bound invariants, consistent with the adapter-not-authority law established in v163.x. The Rydberg array tomography signal grounds the GF(d) encoding policy realism and system-size limitation requirements without granting hardware authority.
...
MIDI sequences and PCM render specifications are deterministic artifacts when their generation policy is declared. They are not musical compositions. They are structured event streams derived from canonical proof state. The determinism of a MIDI event stream depends on: event timestamp policy (tick-based or sample-based), tempo declaration, channel assignment policy, velocity quantization policy, and note-off policy. Any undeclared policy is a source of replay drift.

PCM render specifications are deterministic only when the following are declared: sample rate (Hz), bit depth, channel count, byte order (little-endian or big-endian), dithering policy, and clock source policy. Floating-point intermediate render buffers introduce precision drift unless the precision format is declared via a ReducedPrecisionAdapterReceipt from v178.5.x. Sub-millisecond timing claims in audio event streams require hardware clock declaration and are subject to the same latency envelope discipline as v177.x control-plane receipts.

Research on low-latency inference for quantum-ready RF sensing (Gokhale et al., arXiv:2404.17962, 2024) demonstrates that sub-millisecond inference latency is achievable for signal classification but depends critically on hardware type (GPU vs CPU) and inference framework. This is a source-grounded signal that timing claims in telemetry systems require explicit hardware and framework declarations. Any TelemetryReplayEquivalenceReceipt that claims sub-millisecond event stream generation must declare: hardware type, clock source, and inference framework. The 100x+ latency reduction claim in that work is a benchmark signal, not a universal guarantee. It requires a benchmark receipt with declared hardware and corpus.
...
Event timestamps are declared.
PCM precision is declared.
Clock source is declared.

Sub-millisecond timing claims require hardware declaration.
Floating-point render buffers require precision receipts."
```
...
+ same MIDI event generation policy
  (timestamp policy, tempo, channel assignment, velocity quantization, note-off policy)
+ same PCM render specification
  (sample rate, bit depth, channel count, byte order, dithering policy, clock source)
+ same floating-point precision declaration for render buffers
...
- pytest: MIDIEventStreamReceipt declares timestamp policy (tick-based or sample-based), tempo, channel assignment, velocity quantization, and note-off policy
- pytest: MIDIEventStreamReceipt is hash-stable across PYTHONHASHSEED values
- pytest: PCMRenderSpecReceipt declares sample rate, bit depth, channel count, byte order, dithering policy, and clock source policy
- pytest: PCMRenderSpecReceipt triggers ReducedPrecisionAdapterReceipt from v178.5.x if floating-point render buffers are used
...
- pytest: any sub-millisecond timing claim in TelemetryReplayEquivalenceReceipt must declare hardware type and clock source
...
- pytest: no undeclared MIDI timestamp policy
- pytest: no undeclared PCM byte order
- dependency boundary: no import from src/qec/decoder/
- dependency boundary: audio backends are adapters, never authorities
- dependency boundary: hardware clock sources are adapter signals, not authorities
...
- no undeclared MIDI timestamp policy
- no undeclared PCM byte order or dithering policy
...
- no sub-millisecond timing claim without hardware declaration
- no floating-point render buffer without precision receipt
...
- depends on: v178.5.x IEEE 754 precision receipts (floating-point render buffer precision)
...
- source signals: Gokhale et al., arXiv:2404.17962 (2024) — latency-optimized inference (adapter signal; hardware declaration required for any latency claim)
- does not modify: src/qec/decoder/

**Relationship to Existing Roadmap**
v168.x formalizes the sonification discipline established in v82.3.0, v96.1.0, v136.8.8, and v137.7.2. It does not replace those earlier layers. It wraps their behavioral contracts as receipt-bound proof telemetry artifacts, consistent with the adapter-not-authority law. The MIDI timestamp policy and PCM byte order requirements harden replay semantics against the most common sources of cross-environment event stream drift.
...
Sigma threshold discipline is not optional. A sigma threshold is a declared numerical confidence level (e.g., 3σ, 5σ) derived from a declared statistical test applied to a declared dataset. It is not a vague confidence label. A CosmologicalClaimBoundaryReceipt must declare: the statistical test used, the dataset source, the degrees of freedom, the computed test statistic, and the resulting sigma value. A claim that does not meet its declared sigma threshold is labelled BELOW_THRESHOLD and may not enter the proof pipeline as evidence.

Mathematical identity candidates require a distinct treatment from cosmological signals. A MathematicalIdentityCandidateReceipt must declare: the symbolic expression in canonical form, the verification method (symbolic algebra system, numerical spot-check, or formal proof), the verification scope (finite domain, asymptotic, or universal), and the review status. A symbolic algebra system verification is not a formal proof. A numerical spot-check is not a proof. These are characterization artifacts. The receipt declares the verification method. The receipt does not promote the candidate to theorem status.

The claim_mode = SYMBOLIC_ONLY gate is not merely a label. It is an enforcement boundary. Any symbolic expression that carries claim_mode = SYMBOLIC_ONLY must be rejected by any downstream proof pipeline component that requires claim_mode = VERIFIED or claim_mode = PROVEN. The gate is enforced at receipt validation time, not at human review time.
...
The sigma threshold is a declared numerical value
from a declared statistical test on a declared dataset.
It is not a vague confidence label.

The claim_mode = SYMBOLIC_ONLY gate is enforced
at receipt validation time.
It is not a courtesy label.

The receipt is the boundary.
The claim_mode gate is the enforcement."
```
...
+ same sigma threshold (for cosmological signals)
  (declared statistical test, dataset source, degrees of freedom,
   test statistic, sigma value)
+ same verification method (for mathematical identity candidates)
  (symbolic algebra system, numerical spot-check, or formal proof)
...
- pytest: MathematicalIdentityCandidateReceipt declares symbolic expression in canonical form, verification method, verification scope, and review status
- pytest: MathematicalIdentityCandidateReceipt rejects theorem claims without proof receipt
- pytest: MathematicalIdentityCandidateReceipt labels symbolic algebra system verification as CHARACTERIZATION, not PROOF
- pytest: MathematicalIdentityCandidateReceipt labels numerical spot-check as CHARACTERIZATION, not PROOF
- pytest: CosmologicalClaimBoundaryReceipt declares statistical test, dataset source, degrees of freedom, test statistic, and sigma value
- pytest: CosmologicalClaimBoundaryReceipt labels claims below declared sigma threshold as BELOW_THRESHOLD
- pytest: SymbolicSandboxReplayReceipt verifies same symbolic input produces same canonical output
- pytest: claim_mode = SYMBOLIC_ONLY is enforced at receipt validation time, not only at human review time
- pytest: downstream pipeline components reject claim_mode = SYMBOLIC_ONLY artifacts when claim_mode = VERIFIED is required
...
- pytest: no sigma threshold accepted as vague confidence label
- dependency boundary: no import from src/qec/decoder/
...
- no sigma threshold accepted as vague confidence label
- no symbolic expression without claim_mode declaration
- no undeclared review status
- no claim_mode = SYMBOLIC_ONLY treated as merely advisory
- no symbolic algebra system verification promoted to proof status
- no numerical spot-check promoted to proof status
- no BELOW_THRESHOLD claim entering proof pipeline as evidence
...
**Relationship to Existing Roadmap**
v169.x extends the claim-audit discipline of v137.10.x into the symbolic geometry and cosmological signal domain. It enforces the Mathematical Discovery Reminder already present in the roadmap: no theorem claim without proof receipt, no numerology as proof, no model-generated theorem authority. The sigma threshold discipline is hardened from a label requirement to a numerical enforcement gate. The claim_mode = SYMBOLIC_ONLY gate is hardened from advisory to enforcement.
...
Rydberg atom-based sensors provide a concrete photonic adapter context. Somaweera et al. (Photonics 12(12):1228, 2025) review EIT-based Rydberg sensors operating from DC to THz via electromagnetically induced transparency (EIT) and Autler-Townes (AT) splitting. The review identifies explicit uncertainty sources that must be declared in any receipt referencing Rydberg sensor outputs: atomic decoherence, Doppler and collisional line broadening, laser power stability, cell field distortions, and sensor integration challenges. SI-traceability claims require declaration of the atomic constant source and calibration method. Superheterodyne readout, multi-photon EIT, and multi-wave mixing architectures each introduce distinct signal processing boundaries that must be declared separately.

Infleqtion's Quantum Spectrum platform (Quantum Computing Report, 2026-05-14) provides a source-grounded signal for atom-based RF sensing as a hardware adapter context. The QOBRA project (Australia) uses ML to optimize Rydberg atom-based receivers for sensitivity and instantaneous bandwidth. This is a source-grounded signal that ML optimization of Rydberg receivers requires benchmark boundary receipts. The frequency range claim (Hz to THz via single aperture) is a hardware signal, not a QEC proof. Any receipt referencing Quantum Spectrum must declare: adapter_only=true, source URL, and the specific frequency range claim with its source.
...
EIT-based readout is a photonic adapter signal.
AT splitting is a measurement boundary.
Rydberg atom RF sensing is a hardware adapter signal.
ML-optimized Rydberg receivers require benchmark receipts.

Physical learning language is a metaphor
unless bounded by MaterialsMemoryClaimBoundaryReceipt.

Atomic decoherence, line broadening, and laser instability
are declared uncertainty sources, not silent limitations.
...
- v172.8 → RydbergSensorBoundaryReceipt
- v172.9 → EITReadoutBoundaryReceipt
...
- src/qec/analysis/rydberg_sensor_boundary_receipts.py
- src/qec/analysis/eit_readout_boundary_receipts.py
...
- RydbergSensorBoundaryReceipt
- EITReadoutBoundaryReceipt
...
- rydberg_sensor_boundary_receipt_hash           (v172.8)
- eit_readout_boundary_receipt_hash              (v172.9)
...
+ same declared uncertainty sources
  (for Rydberg sensors: decoherence model, line broadening source,
   laser stability declaration, cell field distortion policy)
...
- pytest: RydbergSensorBoundaryReceipt declares sensing modality (EIT/AT/Stark), frequency range source, and adapter_only=true
- pytest: RydbergSensorBoundaryReceipt declares atomic decoherence model, line broadening source, and laser stability declaration
- pytest: RydbergSensorBoundaryReceipt rejects SI-traceability claims without declared atomic constant source and calibration method
- pytest: RydbergSensorBoundaryReceipt requires benchmark receipt for any ML-optimized receiver sensitivity claim
- pytest: EITReadoutBoundaryReceipt declares probe laser parameters, coupling laser parameters, and readout architecture (standard EIT, superheterodyne, multi-photon, or multi-wave mixing)
- pytest: EITReadoutBoundaryReceipt declares cell field distortion policy and adapter_only=true
...
- dependency boundary: Rydberg sensor outputs are adapter signals, not QEC authority
...
- no claim scope expansion beyond declared evidence
- no SI-traceability claim without atomic constant source and calibration method
- no Rydberg sensor sensitivity claim without declared uncertainty sources
- no ML-optimized receiver claim without benchmark receipt
- no EIT readout claim without declared laser parameters and readout architecture
- no frequency range claim without source declaration
...
- source signals: Somaweera et al., Photonics 12(12):1228 (2025) — Rydberg sensor review (adapter signal; adapter_only=true)
- source signals: Infleqtion Quantum Spectrum announcement (2026-05-14) — atom-based RF sensing (adapter signal; adapter_only=true)
- does not modify: src/qec/decoder/

**Relationship to Existing Roadmap**
v172.x extends the substrate constraint discipline of v158.x and the quantum hardware signal discipline of v165.9.x into photonic, graphene, and diamond device contexts. It enforces the Materials Signal Reminder already present in the roadmap. The addition of RydbergSensorBoundaryReceipt and EITReadoutBoundaryReceipt grounds the photonic adapter discipline in the specific uncertainty sources identified in the Rydberg sensor literature, without granting hardware authority.
...
Atom-based RF sensing systems provide a concrete control-plane adapter context. The RIQER project (Infleqtion / Army Research Laboratory, 2026) is developing a transportable quantum RF demonstration system for positioning, navigation, and timing (PNT). This is a source-grounded signal that field-deployable quantum hardware systems require explicit latency envelope receipts and hardware adapter boundary declarations. The transportable form factor introduces additional environmental perturbation sources (vibration, temperature, electromagnetic interference) that must be declared in a HardwareAdapterBoundaryReceipt. The PNT application context introduces safety-critical automation risks that must be explicitly excluded from QEC control-plane receipts.

Low-latency quantum-ready RF sensing research (Gokhale et al., arXiv:2404.17962, 2024) demonstrates that sub-millisecond inference latency is achievable for RF signal classification but depends on hardware type (GPU vs CPU) and inference framework. This is a source-grounded signal that LatencyEnvelopeReceipts must declare: hardware type, inference framework, and measurement method. The 100x+ latency reduction claim requires a benchmark receipt with declared baseline comparator and hardware.
...
with declared hardware type and measurement method.
The fidelity model is a source-bound signal.

Field-deployable hardware introduces
environmental perturbation sources
that must be declared, not assumed stable.

No live hardware authority.
No safety-critical automation.
No PNT application authority.
The receipt is the boundary."
```
...
+ same latency envelope declaration
  (hardware type, inference framework, measurement method,
   environmental perturbation sources for field-deployable hardware)
...
- pytest: HardwareAdapterBoundaryReceipt declares environmental perturbation sources for field-deployable hardware (vibration, temperature, EMI)
- pytest: LatencyEnvelopeReceipt declares measured latency distribution, hardware type, inference framework, and measurement method
- pytest: LatencyEnvelopeReceipt rejects sub-millisecond claims without declared hardware type and measurement method
...
- pytest: no safety-critical automation
- pytest: no real-time guarantee without measurement receipt
- pytest: no PNT application authority claims
- pytest: no hardware authority claims
- dependency boundary: no import from src/qec/decoder/
- dependency boundary: hardware backends are adapters, never authorities
- dependency boundary: field-deployable hardware systems are adapter contexts, not QEC authorities
...
- no real-time guarantee without measurement receipt
- no hardware authority claim
- no PNT application authority claim
- no fidelity model without source declaration
- no noise model without source declaration
- no latency claim without measurement receipt
- no latency claim without hardware type declaration
- no control-plane interaction without receipt
- no environmental perturbation sources omitted for field-deployable hardware
...
- source signals: Infleqtion RIQER project (2026) — transportable quantum RF system (adapter signal; adapter_only=true)
- source signals: Gokhale et al., arXiv:2404.17962 (2024) — latency-optimized inference (adapter signal; hardware declaration required)
- does not modify: src/qec/decoder/

**Relationship to Existing Roadmap**
v177.x implements the `control_plane_manifest_hash` entry in the terminal proof chain. It enforces the Hardware Profile Reminder already present in the roadmap: hardware profiles are adapter contexts, not authorities. The Real-Time Control Reminder is also enforced: no live control authority, no safety-critical automation. The environmental perturbation source requirement grounds the latency envelope discipline in the specific challenges of field-deployable quantum hardware systems.
...
Rydberg array experiments provide a concrete contextuality adapter context. Mukherjee et al. (Phys. Rev. A 111, 043319, 2025) demonstrate that controllable decoherence in Rydberg arrays can be modeled via ML-extracted decoherence operators. Decoherence-induced contextuality is a research signal: the presence of decoherence operators in a quantum system may affect contextuality classifications. However, a contextuality classification derived from a decoherence model that was itself extracted by ML from a small Rydberg array (few sites) must declare: the decoherence model source, the ML extraction method, the system size, and the training corpus size. The steep training data requirement identified in that work means that contextuality classifications derived from small-array decoherence models carry a system-size limitation that must be declared in the ContextualityThresholdReceipt.

Topological boundary discipline requires explicit declaration of: code family (surface code, color code, toric code, hypergraph product code, etc.), boundary condition type (smooth, rough, mixed, periodic), stabilizer generator type (X-type, Z-type, or mixed), and logical operator representative. A TopologicalBoundaryReceipt that does not declare these fields is incomplete. Code-switching protocols require additional declaration of: switching trigger condition, intermediate code family, overhead source, and rollback condition.
...
Decoherence-induced contextuality requires
a declared decoherence model source
and a declared system-size limitation.

Topological boundaries are declared constraints
with declared code family, boundary condition type,
and logical operator representative.
...
+ same topological boundary declaration
  (code family, boundary condition type, stabilizer generator type,
   logical operator representative)
+ same geometry signal source
+ same code-switching policy
  (switching trigger, intermediate code family, overhead source, rollback condition)
+ same decoherence model source (when decoherence-induced contextuality is claimed)
...
- pytest: ContextualityThresholdReceipt declares decoherence model source and system-size limitation when decoherence-induced contextuality is claimed
- pytest: ContextualityThresholdReceipt rejects contextuality claims derived from ML-extracted decoherence models without declared system-size limitation
- pytest: TopologicalBoundaryReceipt declares code family, boundary condition type, stabilizer generator type, and logical operator representative
- pytest: TopologicalBoundaryReceipt declares constraint source
- pytest: GeometryReplayValidationReceipt verifies same geometry signal produces same canonical output
- pytest: CodeSwitchingBoundaryReceipt declares switching trigger, intermediate code family, overhead source, and rollback condition
...
- pytest: no topological boundary without declared code family and boundary condition type
- pytest: no code-switching without boundary receipt
- pytest: no code-switching without declared rollback condition
- dependency boundary: no import from src/qec/decoder/
- dependency boundary: geometry backends are adapters, never authorities
- dependency boundary: Rydberg array decoherence models are adapter signals, not contextuality authorities
...
- no topological boundary without code family declaration
- no code-switching without boundary receipt
- no code-switching without rollback condition
- no geometry signal without adapter_only declaration
- no cosmological geometry claim
- no decoherence-induced contextuality claim without declared decoherence model source
- no system-size limitation omitted from contextuality receipts referencing small-array evidence
...
- source signals: Mukherjee et al., Phys. Rev. A 111, 043319 (2025) — Rydberg array decoherence modeling (adapter signal; system-size limitation declared)
- does not modify: src/qec/decoder/

**Relationship to Existing Roadmap**
v179.x implements the `contextuality_threshold_receipt_hash` entry in the terminal proof chain. It enforces the Contextuality Reminder already present in the roadmap: contextuality classifications are thresholded, not ontological. The three seed releases already appear in the roadmap; this section completes them. The decoherence model source requirement and system-size limitation requirement ground the contextuality threshold discipline in the specific limitations identified in the Rydberg array tomography literature.
...
Research on quantum-ready ML for RF sensing (Gokhale et al., arXiv:2404.17962, 2024) provides a concrete QML boundary context. The paper validates quantum-readiness through physics-based simulation of Rydberg atom QRF sensors, not through actual quantum hardware execution. This is a source-grounded signal that "quantum-ready" means simulation-validated, not hardware-proven. Any QMLBoundaryManifest that references quantum-ready ML must declare: simulation framework, simulation fidelity model, and the explicit distinction between simulation-validated and hardware-validated claims.

Research on quantum network tomography of Rydberg arrays by ML (Mukherjee et al., Phys. Rev. A 111, 043319, 2025) demonstrates ML applied to quantum systems for Hamiltonian extraction. The ML model is trained on simulation data and applied to experimental data. This simulation-to-experiment transfer introduces a transfer boundary that must be declared in a QMLPrecisionBoundaryReceipt: training corpus source (simulation or experiment), transfer policy (direct application or fine-tuning), and transfer fidelity declaration. The steep training data requirement (best suited for few-site networks) must be declared as a system-size limitation.

Shot count and seed policy discipline is not merely a formality. In quantum circuit sampling, the shot count determines the statistical resolution of the output distribution. A QMLReplayEquivalenceReceipt that claims same circuit + same seed → same output distribution must declare: the statistical distance metric used for distribution comparison (total variation distance, KL divergence, or Hellinger distance), the declared tolerance, and the shot count. Distributions are not byte-identical; they are statistically equivalent under a declared metric and tolerance.
...
'Quantum-ready' means simulation-validated,
not hardware-proven.
The simulation framework is declared.
The simulation fidelity model is declared.

The shot count is declared.
The seed is declared.
The precision format is declared.
The statistical distance metric is declared.
The receipt is the boundary."
```
...
+ same statistical distance metric and tolerance
  (for distribution comparison in replay equivalence)
+ same simulation framework and fidelity model
  (when quantum-ready claims reference simulation validation)
...
- pytest: QMLBoundaryManifest declares simulation framework and fidelity model when quantum-ready claims reference simulation validation
- pytest: QMLBoundaryManifest explicitly distinguishes simulation-validated from hardware-validated claims
...
- pytest: QMLPrecisionBoundaryReceipt declares training corpus source (simulation or experiment) and transfer policy for simulation-to-experiment transfer
- pytest: QMLPrecisionBoundaryReceipt declares system-size limitation when referencing small-array evidence
- pytest: QMLBenchmarkBoundaryReceipt rejects any performance claim without benchmark receipt
- pytest: QMLReplayEquivalenceReceipt declares statistical distance metric, tolerance, and shot count for distribution comparison
- pytest: QMLReplayEquivalenceReceipt does not claim byte-identical distribution equivalence; it claims statistical equivalence under declared metric
- pytest: QuantumAdvantageClaimBoundaryReceipt requires source-bound evidence and sigma threshold
- pytest: QuantumAdvantageClaimBoundaryReceipt rejects simulation-validated claims as hardware-proven
...
- dependency boundary: simulation-validated quantum-readiness ≠ hardware-proven quantum advantage
...
- no hardware authority claim
- no simulation-validated claim promoted to hardware-proven status
- no distribution comparison without declared statistical distance metric
- no system-size limitation omitted from receipts referencing small-array evidence
- no simulation-to-experiment transfer without declared transfer policy
...
- source signals: Gokhale et al., arXiv:2404.17962 (2024) — quantum-ready ML for RF sensing (adapter signal; simulation-validated, not hardware-proven)
- source signals: Mukherjee et al., Phys. Rev. A 111, 043319 (2025) — Rydberg array ML tomography (adapter signal; system-size limitation declared)
- does not modify: src/qec/decoder/

**Relationship to Existing Roadmap**
v180.x implements the `qml_boundary_receipt_hash` entry in the terminal proof chain. It enforces the Contextuality Reminder and the Inference Optimization Reminder: no hardware authority claims, no quantum advantage claims without source-bound evidence. The simulation-validated vs hardware-proven distinction and the statistical distance metric requirement harden the QML boundary discipline against the most common sources of quantum advantage overclaiming.
...
Benchmark governance realism requires explicit suppression of three specific failure modes that are common in quantum computing benchmarking:

First, comparator scope inflation: a benchmark that compares QEC performance against a classical baseline on a problem that is not representative of the claimed application domain. Every ComparatorIdentityReceipt must declare the comparator's application domain and the explicit scope of the comparison.

Second, hardware heterogeneity suppression: a benchmark that claims performance improvement without declaring the hardware difference between the QEC system and the comparator. Any benchmark that uses different hardware for the QEC system and the comparator must declare both hardware types and produce a HardwareBenchmarkDeclarationReceipt for each. A benchmark that uses GPU acceleration for the QEC system and CPU for the comparator without declaring this difference is non-compliant.

Third, corpus selection bias: a benchmark corpus that was selected to favor the QEC system. Every BenchmarkCorpusReceipt must declare the corpus selection method (random, adversarial, representative, or curated) and the selection rationale. A corpus that was selected after seeing the QEC system's performance is non-compliant.

Research on low-latency quantum-ready RF sensing (Gokhale et al., arXiv:2404.17962, 2024) provides a concrete benchmark governance context. The paper reports 100x+ latency reductions for inference. This is a source-grounded signal that multi-order-of-magnitude performance claims require: declared baseline comparator identity, declared hardware type for both systems, declared inference framework for both systems, declared corpus, and declared measurement method. The 100x+ claim is a benchmark signal, not a universal guarantee. Any BenchmarkMeasurementReceipt referencing this work must declare: GPU vs CPU comparison, inference framework versions, and corpus characteristics.
...
The hardware is declared for both systems.
The claim scope is declared.

Comparator scope inflation is non-compliant.
Hardware heterogeneity suppression is non-compliant.
Corpus selection bias is non-compliant.
...
+ same benchmark corpus
  (selection method declared: random, adversarial, representative, or curated)
+ same hardware declaration for both QEC system and comparator
+ same measurement method
+ same claim scope declaration
  (application domain declared; scope inflation rejected)
...
- pytest: ComparatorIdentityReceipt declares comparator name, version, source, application domain, and adapter_only status
- pytest: BenchmarkCorpusReceipt declares corpus name, version, size, source hash, and selection method
- pytest: BenchmarkCorpusReceipt rejects corpora selected after seeing QEC system performance
- pytest: HardwareBenchmarkDeclarationReceipt declares hardware type, OS, and measurement environment for both QEC system and comparator
- pytest: HardwareBenchmarkDeclarationReceipt rejects benchmarks that use GPU for QEC system and CPU for comparator without declaring the difference
- pytest: BenchmarkMeasurementReceipt declares measurement method, repetition count, and statistical summary
- pytest: BenchmarkMeasurementReceipt declares inference framework versions for both systems when comparing inference latency
...
- pytest: BenchmarkClaimScopeReceipt rejects comparator scope inflation
...
- no hardware authority claim
- no claim scope expansion beyond declared evidence
- no benchmark claim without hardware declaration for both systems
- no performance marketing language
- no quantum advantage claim from benchmark alone
- no comparator scope inflation
- no hardware heterogeneity suppression
- no corpus selection bias
- no multi-order-of-magnitude claim without declared baseline comparator and hardware
...
- source signals: Gokhale et al., arXiv:2404.17962 (2024) — latency benchmark governance (hardware declaration required; corpus declaration required)
- does not modify: src/qec/decoder/

**Relationship to Existing Roadmap**
v184.x implements the `benchmark_ladder_receipt_hash` entry in the terminal proof chain. It enforces the Benchmark Reminder already present in the roadmap: no benchmark claims without comparator receipts. It is identified as a Zenodo publication opportunity. The three governance failure modes (comparator scope inflation, hardware heterogeneity suppression, corpus selection bias) are added as explicit non-compliance conditions.
...
Rydberg atom-based sensors provide a concrete v2 photonic device context. Somaweera et al. (Photonics 12(12):1228, 2025) review EIT-based Rydberg sensors with superheterodyne readout, multi-photon EIT, and multi-wave mixing architectures. These architectures each introduce distinct signal processing boundaries. A v2 IntegratedPhotonicCircuitReceipt that references Rydberg sensor integration must declare: the specific EIT architecture (standard, superheterodyne, multi-photon, or multi-wave mixing), the photonic integration approach (vapor cell, chip-integrated, or fiber-coupled), and the declared uncertainty sources (decoherence, line broadening, laser stability, cell field distortions).

The Infleqtion Quantum Spectrum platform (Quantum Computing Report, 2026-05-14) demonstrates atom-based RF sensing with claimed Hz-to-THz coverage via single aperture. This is a source-grounded signal for v2 photonic device receipts. Any receipt referencing this platform must declare: adapter_only=true, the specific frequency range claim with its source, and the explicit distinction between laboratory demonstration and field-deployable performance.
...
Rydberg sensor integration is an adapter signal.

EIT architecture must be declared.
Photonic integration approach must be declared.
Uncertainty sources must be declared.

Laboratory demonstration ≠ field-deployable performance.
...
- v185.7 → RydbergSensorIntegrationReceiptV2
...
- src/qec/analysis/rydberg_sensor_integration_receipts_v2.py
...
- RydbergSensorIntegrationReceiptV2
...
- rydberg_sensor_integration_receipt_v2_hash     (v185.7)
...
+ same EIT architecture declaration (for Rydberg sensor integration)
+ same photonic integration approach declaration
+ same declared uncertainty sources
...
- pytest: IntegratedPhotonicCircuitReceipt declares EIT architecture and photonic integration approach when referencing Rydberg sensor integration
...
- pytest: RydbergSensorIntegrationReceiptV2 declares EIT architecture, photonic integration approach, and declared uncertainty sources
- pytest: RydbergSensorIntegrationReceiptV2 explicitly distinguishes laboratory demonstration from field-deployable performance
- pytest: RydbergSensorIntegrationReceiptV2 requires benchmark receipt for any ML-optimized sensitivity claim
- pytest: inaccessible sources are marked source_inaccessible
- pytest: no hardware authority claims
...
- no inaccessible source as sole phase justification
- no hardware authority claim
- no EIT architecture omitted from Rydberg sensor integration receipts
- no photonic integration approach omitted from Rydberg sensor integration receipts
- no laboratory demonstration promoted to field-deployable performance claim
- no ML-optimized sensitivity claim without benchmark receipt
...
- source signals: Somaweera et al., Photonics 12(12):1228 (2025) — Rydberg sensor review (adapter signal; EIT architecture declaration required)
- source signals: Infleqtion Quantum Spectrum (2026-05-14) — atom-based RF sensing (adapter signal; laboratory vs field-deployable distinction required)
- does not modify: src/qec/decoder/

**Relationship to Existing Roadmap**
v185.x implements the `materials_signal_boundary_receipt_v2_hash` entry in the terminal proof chain. It extends the v172.x materials signal discipline with the device-level signal contexts that emerge from the v177.x–v179.x arc cluster. The addition of RydbergSensorIntegrationReceiptV2 grounds the photonic adapter discipline in the specific EIT architecture and uncertainty source requirements identified in the Rydberg sensor literature.
...
Floating-point environment drift is the primary source of cross-environment replay failure in scientific computing. The IEEE 754 precision receipts from v178.5.x establish the boundary discipline for floating-point operations. v189.x extends that discipline into the cross-environment context. A FloatingPointEnvironmentReceipt must declare: FPU type (x87, SSE2, AVX, ARM NEON, or other), subnormal handling policy (flush-to-zero or IEEE 754 compliant), NaN propagation policy, rounding mode (round-to-nearest-even, round-toward-zero, round-up, or round-down), and BLAS implementation (OpenBLAS, MKL, LAPACK, or other). Any difference in these declarations between environments constitutes a floating-point environment drift event and must produce an EnvironmentDriftReceipt.

ML inference timing is environment-sensitive. Research on low-latency quantum-ready RF sensing (Gokhale et al., arXiv:2404.17962, 2024) demonstrates that sub-millisecond inference latency depends on hardware type (GPU vs CPU). This is a source-grounded signal that any cross-environment replay that includes ML inference timing must declare: hardware type, CUDA version (for GPU), and inference framework version. Sub-millisecond timing claims that do not declare these fields are non-compliant.
...
The floating-point environment is declared:
  FPU type, subnormal policy, NaN policy,
  rounding mode, BLAS implementation.
...
Any floating-point environment difference
→ EnvironmentDriftReceipt produced
→ not silent fallback.

Sub-millisecond timing claims require
hardware type and CUDA version declaration."
```
...
+ same floating-point environment declaration
  (FPU type, subnormal handling, NaN propagation,
   rounding mode, BLAS implementation)
...
Any sub-millisecond timing claim
→ hardware type and CUDA version declared
→ inference framework version declared
```
...
- pytest: HardwareEnvironmentBoundaryReceipt declares hardware type, FPU type, and adapter_only=true
- pytest: HardwareEnvironmentBoundaryReceipt declares CUDA version for GPU environments
...
- pytest: FloatingPointEnvironmentReceipt declares FPU type, subnormal handling policy, NaN propagation policy, rounding mode, and BLAS implementation
- pytest: FloatingPointEnvironmentReceipt is required for any environment that performs floating-point computation
- pytest: EnvironmentDriftReceipt is produced on any floating-point environment difference between declared environments
...
- pytest: sub-millisecond timing claims require hardware type, CUDA version, and inference framework version
- dependency boundary: no import from src/qec/decoder/
...
- no floating-point environment without FPU type declaration
- no floating-point environment without BLAS implementation declaration
...
- no sub-millisecond timing claim without hardware type and CUDA version declaration
- no flush-to-zero subnormal policy silently applied
...
- source signals: Gokhale et al., arXiv:2404.17962 (2024) — hardware-sensitive timing (GPU vs CPU declaration required)
- does not modify: src/qec/decoder/

**Relationship to Existing Roadmap**
v189.x implements the `cross_environment_replay_receipt_hash` entry in the terminal proof chain. It is identified as a Zenodo publication opportunity. It extends the cross-environment replay discipline of v148.6 and v151.8 into hardware and OS contexts. The FloatingPointEnvironmentReceipt is hardened with explicit FPU type, subnormal handling, NaN propagation, rounding mode, and BLAS implementation requirements, grounding the replay discipline in the specific sources of cross-environment floating-point drift.
...
Hermetic environment realism requires explicit treatment of three categories of hidden non-hermeticity that are common in scientific computing environments:

First, implicit system library dependencies: Python packages that link against system libraries (BLAS, LAPACK, libssl, libffi) without declaring those dependencies in the pin manifest. A HermeticDependencyPinReceipt must declare not only Python package versions but also the system library versions that those packages link against. This requires a declared system library inventory method (ldd output, conda environment export, or container image manifest).

Second, filesystem timestamp non-determinism: build systems that embed filesystem timestamps in output artifacts. A HermeticBuildReceipt must declare the timestamp normalization policy (SOURCE_DATE_EPOCH, fixed timestamp, or content-addressed). Any build artifact that embeds a filesystem timestamp without declared normalization is non-hermetic.

Third, network-fetched build dependencies: build systems that fetch dependencies from the network during the build process. A HermeticBuildReceipt must declare the network isolation policy for the build phase. Any build that fetches from the network without declaring the fetch URL, expected hash, and fallback policy is non-hermetic.
...
System library versions are declared.
Timestamp normalization is declared.
Network isolation is declared.

No undeclared external dependency.
No implicit system library dependency.
No filesystem timestamp without normalization policy.
No network fetch without declared URL and hash.
...
+ same pinned build environment
  (Python packages, system libraries, timestamp normalization policy)
+ same pinned execution environment
  (FPU type, BLAS implementation, locale, timezone)
+ same network isolation policy
  (allowed endpoints, fetch URL/hash declarations)
+ same filesystem isolation policy
  (allowed read/write paths, side-effect policy)
+ same dependency pin manifest
  (Python packages + system libraries + declared inventory method)
...
- pytest: HermeticBuildReceipt declares source hash, dependency pin manifest hash, build environment hash, and timestamp normalization policy
- pytest: HermeticBuildReceipt rejects build artifacts with embedded filesystem timestamps without declared normalization
- pytest: HermeticBuildReceipt declares network isolation policy for build phase
- pytest: HermeticExecutionReceipt declares execution environment hash, network isolation policy, filesystem scope, FPU type, and BLAS implementation
- pytest: NetworkIsolationPolicyReceipt declares allowed network endpoints (empty list for fully hermetic) and fetch URL/hash declarations for any network fetch
- pytest: FilesystemIsolationPolicyReceipt declares allowed read/write paths and side-effect policy
- pytest: HermeticDependencyPinReceipt declares all Python packages with version and source hash
- pytest: HermeticDependencyPinReceipt declares system library versions and inventory method (ldd, conda, or container manifest)
...
- pytest: no implicit system library dependency
- pytest: no filesystem timestamp without normalization policy
- pytest: no network fetch without declared URL and hash
...
- no system library version omitted from dependency pin manifest
...
Composition semantics require explicit treatment of three failure modes that are specific to the composition phase:

First, claim scope laundering: an artifact that carries a narrow claim scope (e.g., simulation-validated) is composed with a broader claim scope artifact (e.g., hardware-proven) and the composition receipt inherits the broader scope without explicit declaration. Every SourceClaimCompositionReceipt must declare the minimum claim scope across all composed artifacts. The composition scope is the minimum, not the maximum.

Second, adapter authority laundering: an artifact that carries adapter_only=true is composed into a receipt that does not carry adapter_only=true, silently promoting the adapter to authority status. Every composition receipt that includes any adapter_only=true artifact must itself carry adapter_only=true unless an explicit promotion receipt exists.

Third, review status laundering: an artifact that carries UNREVIEWED_PREPRINT status is composed with a reviewed artifact and the composition receipt inherits the reviewed status. Every composition receipt must carry the minimum review status across all composed artifacts. UNREVIEWED_PREPRINT status is not laundered by composition.
...
Composition scope is the minimum, not the maximum.
Adapter status is not laundered by composition.
Review status is not laundered by composition.

Every registered artifact has:
  - declared source
  - declared claim scope
  - declared review status
  - declared adapter status
  - stable hash
...
+ same source claim receipts
  (composition scope = minimum claim scope across all composed artifacts)
+ same benchmark boundary receipts
  (adapter_only status propagated; not laundered)
+ same operator/reproducibility receipts
  (review status = minimum review status across all composed artifacts)
+ same symbolic/interpretability receipts
  (claim_mode = SYMBOLIC_ONLY propagated when any composed artifact carries it)
...
**Composition Scope Laws**

```text
Law 1 — Scope Minimum:
  composition_claim_scope = min(claim_scope for all composed artifacts)
  simulation-validated < hardware-validated
  CHARACTERIZATION < VERIFIED < PROVEN

Law 2 — Adapter Propagation:
  if any composed artifact has adapter_only=true
  then composition receipt has adapter_only=true
  unless explicit promotion receipt exists

Law 3 — Review Status Minimum:
  composition_review_status = min(review_status for all composed artifacts)
  UNREVIEWED_PREPRINT < UNDER_REVIEW < REVIEWED < PUBLISHED

Law 4 — Claim Mode Propagation:
  if any composed artifact has claim_mode = SYMBOLIC_ONLY
  then composition receipt has claim_mode = SYMBOLIC_ONLY
  unless explicit promotion receipt exists
```
...
- pytest: SourceClaimCompositionReceipt declares composition scope as minimum claim scope across all composed artifacts
- pytest: SourceClaimCompositionReceipt rejects claim scope laundering
- pytest: BenchmarkAndHardwareBoundaryCompositionReceipt verifies all benchmark claims have comparator receipts
- pytest: BenchmarkAndHardwareBoundaryCompositionReceipt propagates adapter_only=true from any adapter-only artifact
- pytest: OperatorAndReproducibilityCompositionReceipt verifies all operator receipts and hermetic receipts
- pytest: OperatorAndReproducibilityCompositionReceipt declares composition review status as minimum review status
- pytest: SymbolicAndInterpretabilityCompositionReceipt verifies all symbolic claims have claim_mode = SYMBOLIC_ONLY
- pytest: SymbolicAndInterpretabilityCompositionReceipt propagates claim_mode = SYMBOLIC_ONLY
...
- pytest: GlobalProofCompositionV2 rejects any artifact without declared adapter status
- pytest: GlobalProofCompositionV2 rejects any artifact with claim scope expansion at composition time
- pytest: GlobalProofCompositionV2 enforces all four Composition Scope Laws
...
- pytest: no claim scope laundering
- pytest: no adapter authority laundering
- pytest: no review status laundering
- dependency boundary: no import from src/qec/decoder/
...
- no claim scope laundering
- no adapter authority laundering
- no review status laundering
- no claim_mode = SYMBOLIC_ONLY laundering
- no artifact registration without declared source
- no artifact registration without declared review status
- no artifact registration without declared adapter status
...
**Relationship to Existing Roadmap**
v192.x implements the `global_proof_composition_v2_hash` terminal entry in the proof chain. It is identified as a Zenodo publication opportunity. It extends the global proof composition discipline of v161.x to cover all post-v173 receipt families. The four Composition Scope Laws are added as explicit enforcement rules that prevent the three composition failure modes (claim scope laundering, adapter authority laundering, review status laundering) that would otherwise allow narrow-scope adapter signals to silently acquire broad-scope proof authority.

---

## Source Signal Registry (v2 Addition)

The following source signals from links.md are registered as adapter-only research signals for the phases indicated. All carry adapter_only=true. None may be cited as QEC authority.

```text
[SIGNAL-1] Infleqtion Quantum Spectrum / Atom-Based RF Sensing
  URL: https://quantumcomputingreport.com/infleqtion-introduces-quantum-spectrum-for-atom-based-rf-sensing/
  Date: 2026-05-14
  Used by: v172.x (RydbergSensorBoundaryReceipt), v177.x (LatencyEnvelopeReceipt),
           v185.x (RydbergSensorIntegrationReceiptV2)
  adapter_only: true
  claim_scope: laboratory-demonstration-to-field-deployable-transition-in-progress
  review_status: PUBLISHED (trade press; not peer-reviewed)

[SIGNAL-2] arXiv:2404.17962 — Deep Learning for Low-Latency, Quantum-Ready RF Sensing
  URL: https://arxiv.org/abs/2404.17962
  Authors: Gokhale, Carnahan, Clark, Tomesh, Chong
  Date: 2024-04-27 (revised 2025-04-23)
  Used by: v168.x (TelemetryReplayEquivalenceReceipt), v177.x (LatencyEnvelopeReceipt),
           v184.x (BenchmarkMeasurementReceipt), v189.x (FloatingPointEnvironmentReceipt)
  adapter_only: true
  claim_scope: simulation-validated; not hardware-proven
  review_status: PUBLISHED (arXiv preprint; peer review status to be declared per use)

[SIGNAL-3] Phys. Rev. A 111, 043319 — Quantum Network Tomography of Small Rydberg Arrays by ML
  URL: https://journals.aps.org/pra/abstract/10.1103/PhysRevA.111.043319
  Authors: Mukherjee, Schachenmayer, Whitlock, Wüster
  Date: 2025-04-22
  Used by: v167.x (QuditBenchmarkBoundaryReceipt), v179.x (ContextualityThresholdReceipt),
           v180.x (QMLPrecisionBoundaryReceipt)
  adapter_only: true
  claim_scope: small-system (few sites); simulation-to-experiment transfer
  review_status: PUBLISHED (peer-reviewed journal)
  system_size_limitation: best suited for networks of just a few sites

[SIGNAL-4] MDPI Photonics 12(12):1228 — Rydberg Atom-Based Sensors: Principles, Recent Advances, Applications
  URL: https://www.mdpi.com/2304-6732/12/12/1228
  Authors: Somaweera et al.
  Date: 2025-12-12
  Used by: v172.x (RydbergSensorBoundaryReceipt, EITReadoutBoundaryReceipt),
           v185.x (RydbergSensorIntegrationReceiptV2)
  adapter_only: true
  claim_scope: review article; individual claims require primary source verification
  review_status: PUBLISHED (peer-reviewed journal; open access)
  declared_uncertainty_sources: atomic decoherence, Doppler/collisional line broadening,
    laser power stability, cell field distortions, sensor integration challenges
```
