    ...
    *,
    ...
    sorted_observations = tuple(
        rec for _, rec in sorted(
            ...
            key=lambda item: (
                item[1].theme_index,
                item[1].source_theme_hash,
                item[1].source_replay_identity_hash,
                item[0],
            ),
        )
    )
    expected_sorted = tuple(source_records)
    disordered_indices = tuple(
        sorted(
            {
                obs.theme_index
                for obs, expected in zip(sorted_observations, expected_sorted)
                if obs.theme_index != expected.theme_index
            }
        )
    )
    ...
    fracture_count = len(missing_indices) + len(disordered_indices)
    ...
    if sonification_spec is not None:
        ...
        sonification_metadata = SonificationContinuityRepairMetadata(
            ...
            continuity_repaired=classification != "no_op",
            sonification_lineage_preserved=replay_safe,
            metadata_hash=_sha256_hex(sonification_payload),
        )
    ...
    artifact_payload = {
        ...
        "repaired_records": tuple(record.to_dict() for record in repaired_chain),
        "preserved_theme_hashes": source_artifact.preserved_theme_hashes,
        "continuity_score": continuity_score,
        "recovery_confidence": recovery_confidence,
        "fracture_severity": fracture_severity,
        "lineage_report": lineage_report.to_dict(),
        "sonification_repair_metadata": (
            None if sonification_metadata is None else sonification_metadata.to_dict()
        ),
        "replay_safe": replay_safe,
        "law_invariants": law_invariants,
    }
    recovery_hash = _sha256_hex(artifact_payload)
