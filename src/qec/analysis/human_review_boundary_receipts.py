...
_ALLOWED_REVIEWER_TYPES = {"HUMAN_INDIVIDUAL", "HUMAN_TEAM", "DECLARED_EXTERNAL_REVIEWER", "INTERNAL_REVIEW_GROUP", "UNREVIEWED"}
...
# Review boundary modes that indicate review has not been completed.
_INCOMPLETE_REVIEW_BOUNDARY_MODES = {"NO_REVIEW", "HUMAN_REVIEW_PENDING", "PEER_REVIEW_PENDING"}
...
def _validate_review_semantics(receipt: HumanReviewBoundaryReceipt, upstream: PaperGenerationProvenanceReceipt, manifest: ResearchAutomationManifest) -> tuple[int, bool]:
    ...
    # Enforce canonical ordering and uniqueness of authority boundaries.
    boundary_names = [b.authority_boundary for b in receipt.authority_boundaries]
    if len(boundary_names) != len(set(boundary_names)):
        raise ValueError("authority_boundaries must not contain duplicates")
    if boundary_names != sorted(boundary_names):
        raise ValueError("authority_boundaries must be in canonical sorted order")
    ...
    # UNREVIEWED manifest: review lineage does not support a completed receipt.
    if manifest.review_status.review_status == "UNREVIEWED":
        force_incomplete = True
    # UNREVIEWED reviewer_type: no human reviewer is assigned yet.
    if receipt.reviewer_identity.reviewer_type == "UNREVIEWED":
        force_incomplete = True
    # Incomplete upstream review boundary: honour the upstream paper lineage.
    if upstream.review_boundary.review_boundary_mode in _INCOMPLETE_REVIEW_BOUNDARY_MODES or not upstream.review_boundary.reviewer_required:
        force_incomplete = True
...
def build_review_gap_declaration(gap_index: int, gap_type: str, gap_reason: str) -> ReviewGapDeclaration:
    if isinstance(gap_index, bool) or not isinstance(gap_index, int) or gap_index < 0:
...
def validate_review_gap_declaration(decl: ReviewGapDeclaration) -> bool:
    ...
    if isinstance(decl.gap_index, bool) or not isinstance(decl.gap_index, int) or decl.gap_index < 0:
        raise ValueError("gap_index must be a non-negative int")
...
def build_human_review_boundary_receipt(
    ...
    gap_count, complete = _validate_review_semantics(receipt, paper_generation_provenance_receipt, research_automation_manifest)
...
def validate_human_review_boundary_receipt(
    ...
    if not isinstance(receipt.review_gaps, tuple):
        raise ValueError("review_gaps must be a tuple")
    if not isinstance(receipt.authority_boundaries, tuple):
        raise ValueError("authority_boundaries must be a tuple")
    validate_paper_generation_provenance_receipt(paper_generation_provenance_receipt, research_automation_manifest)
