...
_MAX_ENCODING_LABEL_LENGTH = 200
...
def validate_encoding_entry(entry: EncodingEntry) -> bool:
    try:
        if not isinstance(entry, EncodingEntry):
            raise ValueError(_ERR_INVALID_INPUT)
        
        # Validate all hash fields
        for h in (
            entry.substrate_state_receipt_hash,
            entry.substrate_contract_hash,
            entry.predicate_evaluation_result_hash,
            entry.encoded_value_hash,
            entry.encoding_entry_hash
        ):
            _validate_sha(h)
        
        # Validate encoding index
        if (
            not isinstance(entry.encoding_index, int)
            or isinstance(entry.encoding_index, bool)
            or not (0 <= entry.encoding_index <= _MAX_ENCODING_INDEX)
        ):
            raise ValueError(_ERR_ENCODING_INDEX_OUT_OF_BOUNDS)
        
        # Validate encoding label format
        if (
            not isinstance(entry.encoding_label, str)
            or not entry.encoding_label
            or len(entry.encoding_label) > _MAX_ENCODING_LABEL_LENGTH
            or _LABEL_RE.fullmatch(entry.encoding_label) is None
        ):
            raise ValueError(_ERR_INVALID_ENCODING_LABEL)
        
        # Validate deterministic label derivation
        expected_label = f"ENCODING_{entry.encoding_index:06d}_{entry.predicate_id}"
        if entry.encoding_label != expected_label:
            raise ValueError(_ERR_INVALID_ENCODING_LABEL)
        
        # Validate encoded status
        if entry.encoded_status not in _ALLOWED_ENCODED_STATUSES:
            raise ValueError(_ERR_INVALID_ENCODED_STATUS)
        
        # Recompute and verify hash
        p = _encoding_entry_payload(
            substrate_state_receipt_hash=entry.substrate_state_receipt_hash,
            substrate_contract_hash=entry.substrate_contract_hash,
            substrate_profile_id=entry.substrate_profile_id,
            predicate_evaluation_result_hash=entry.predicate_evaluation_result_hash,
            predicate_id=entry.predicate_id,
            predicate_kind=entry.predicate_kind,
            encoding_index=entry.encoding_index,
            encoding_label=entry.encoding_label,
            encoded_status=entry.encoded_status,
            encoded_value_hash=entry.encoded_value_hash
        )
        if sha256_hex(p) != entry.encoding_entry_hash:
...
def validate_material_encoding_receipt(receipt: MaterialEncodingReceipt) -> bool:
    try:
        if not isinstance(receipt, MaterialEncodingReceipt):
            raise ValueError(_ERR_INVALID_INPUT)
        
        # Validate hash fields
        for h in (
            receipt.substrate_state_receipt_hash,
            receipt.substrate_contract_hash,
            receipt.material_encoding_receipt_hash
        ):
            _validate_sha(h)
        
        # Validate encoding mode
        if receipt.encoding_mode != _ENCODING_MODE:
            raise ValueError(_ERR_INVALID_ENCODING_MODE)
        
        # Validate encoding entries count
        if (
            not isinstance(receipt.encoding_entries, tuple)
            or not (1 <= len(receipt.encoding_entries) <= _MAX_ENCODING_ENTRIES)
        ):
            raise ValueError(_ERR_ENCODING_COUNT_MISMATCH)
        
        # Validate each entry and check for duplicates and cross-field consistency
        ids_i = set()
        ids_h = set()
        ids_p = set()
        sort_key = []
        ...
        for e in receipt.encoding_entries:
            ...
            if e.substrate_state_receipt_hash != receipt.substrate_state_receipt_hash:
                raise ValueError(_ERR_ENCODING_ENTRY_MISMATCH)
            if e.substrate_contract_hash != receipt.substrate_contract_hash:
                raise ValueError(_ERR_ENCODING_ENTRY_MISMATCH)
            if e.substrate_profile_id != receipt.substrate_profile_id:
                raise ValueError(_ERR_ENCODING_ENTRY_MISMATCH)
            
            # Check for duplicates
            if (
                e.encoding_index in ids_i
                or e.predicate_evaluation_result_hash in ids_h
                or e.predicate_id in ids_p
            ):
                raise ValueError(_ERR_DUPLICATE_ENCODING_ENTRY)
            
            ids_i.add(e.encoding_index)
            ids_h.add(e.predicate_evaluation_result_hash)
            ids_p.add(e.predicate_id)
            sort_key.append((
                e.encoding_index,
                e.predicate_id,
                e.predicate_kind,
                e.encoding_entry_hash
            ))
        
        # Validate ordering
        if tuple(sort_key) != tuple(sorted(sort_key)):
            raise ValueError(_ERR_ENCODING_ORDER_MISMATCH)
        
        # Validate count fields
        for c in (
            receipt.encoding_entry_count,
            receipt.passed_encoding_count,
            receipt.failed_encoding_count
        ):
            if not isinstance(c, int) or isinstance(c, bool):
                raise ValueError(_ERR_INVALID_INPUT)
        
        # Validate encoding entry count
        if receipt.encoding_entry_count != len(receipt.encoding_entries):
            raise ValueError(_ERR_ENCODING_COUNT_MISMATCH)
        
        # Recompute and validate passed/failed counts
        passed = sum(1 for e in receipt.encoding_entries if e.encoded_status == "ENCODED_PASS")
        failed = len(receipt.encoding_entries) - passed
        
        if receipt.passed_encoding_count != passed or receipt.failed_encoding_count != failed:
            raise ValueError(_ERR_ENCODING_COUNT_MISMATCH)
        
        # Validate material encoding class
        if receipt.material_encoding_class not in _ALLOWED_MATERIAL_ENCODING_CLASSES:
            raise ValueError(_ERR_MATERIAL_ENCODING_CLASS_MISMATCH)
        
        if receipt.material_encoding_class != _class_from_failed(failed):
            raise ValueError(_ERR_MATERIAL_ENCODING_CLASS_MISMATCH)
        
        # Recompute and verify hash
        p = _material_encoding_receipt_payload(
            substrate_state_receipt_hash=receipt.substrate_state_receipt_hash,
            substrate_contract_hash=receipt.substrate_contract_hash,
            substrate_profile_id=receipt.substrate_profile_id,
            encoding_mode=receipt.encoding_mode,
            encoding_entries=receipt.encoding_entries,
            encoding_entry_count=receipt.encoding_entry_count,
            passed_encoding_count=receipt.passed_encoding_count,
            failed_encoding_count=receipt.failed_encoding_count,
            material_encoding_class=receipt.material_encoding_class
        )
        if sha256_hex(p) != receipt.material_encoding_receipt_hash:
            raise ValueError(_ERR_HASH_MISMATCH)
...
def validate_substrate_drift_receipt(receipt: SubstrateDriftReceipt) -> bool:
    try:
        if not isinstance(receipt, SubstrateDriftReceipt):
            raise ValueError(_ERR_INVALID_INPUT)
        ...
        for h in (
            ...
            receipt.substrate_contract_hash,
            receipt.substrate_drift_receipt_hash
        ):
            _validate_sha(h)
        
        if receipt.observed_material_encoding_receipt_hash is not None:
            _validate_sha(receipt.observed_material_encoding_receipt_hash)
        
        # Validate all hash tuples
        tuples = (
            receipt.expected_encoding_entry_hashes,
            receipt.observed_encoding_entry_hashes,
            receipt.drifted_encoding_entry_hashes,
            receipt.missing_encoding_entry_hashes,
            receipt.unexpected_encoding_entry_hashes
        )
        
        for t in tuples:
            if not isinstance(t, tuple) or len(t) > _MAX_DRIFT_ENTRY_HASHES:
                raise ValueError(_ERR_INVALID_INPUT)
            for h in t:
                _validate_sha(h)
        
        # Check for duplicate-free lists
        for t in tuples:
            if len(t) != len(set(t)):
                raise ValueError(_ERR_INVALID_INPUT)
        
        # Validate internal consistency: subset relationships
        expected_set = set(receipt.expected_encoding_entry_hashes)
        observed_set = set(receipt.observed_encoding_entry_hashes)
        missing_set = set(receipt.missing_encoding_entry_hashes)
        drifted_set = set(receipt.drifted_encoding_entry_hashes)
        unexpected_set = set(receipt.unexpected_encoding_entry_hashes)
        
        # missing and drifted should be subsets of expected
        if not missing_set.issubset(expected_set):
            raise ValueError(_ERR_INVALID_INPUT)
        if not drifted_set.issubset(expected_set):
            raise ValueError(_ERR_INVALID_INPUT)
        
        # unexpected should be subset of observed
        if not unexpected_set.issubset(observed_set):
            raise ValueError(_ERR_INVALID_INPUT)
        
        # When observed is None, observed/unexpected lists should be empty
        if receipt.observed_material_encoding_receipt_hash is None:
            if len(receipt.observed_encoding_entry_hashes) != 0:
                raise ValueError(_ERR_INVALID_INPUT)
            if len(receipt.unexpected_encoding_entry_hashes) != 0:
                raise ValueError(_ERR_INVALID_INPUT)
        ...
        # Validate encoding entry count
        if receipt.encoding_entry_count != len(receipt.expected_encoding_entry_hashes):
            raise ValueError(_ERR_SUBSTRATE_DRIFT_COUNT_MISMATCH)
        
        # Recompute and validate drift count
        drift_count = (
            len(receipt.drifted_encoding_entry_hashes)
            + len(receipt.missing_encoding_entry_hashes)
            + len(receipt.unexpected_encoding_entry_hashes)
        )
        if receipt.drift_count != drift_count:
            raise ValueError(_ERR_SUBSTRATE_DRIFT_COUNT_MISMATCH)
        
        # Validate substrate drift class
        if receipt.substrate_drift_class not in _ALLOWED_SUBSTRATE_DRIFT_CLASSES:
            raise ValueError(_ERR_INVALID_SUBSTRATE_DRIFT_CLASS)
        
        expected_class = _drift_class(
            receipt.observed_material_encoding_receipt_hash,
            receipt.missing_encoding_entry_hashes,
            receipt.unexpected_encoding_entry_hashes,
            receipt.drift_count
        )
        if receipt.substrate_drift_class != expected_class:
            raise ValueError(_ERR_SUBSTRATE_DRIFT_CLASS_MISMATCH)
        
        # Recompute and verify hash
        p = _substrate_drift_receipt_payload(
            expected_material_encoding_receipt_hash=receipt.expected_material_encoding_receipt_hash,
            observed_material_encoding_receipt_hash=receipt.observed_material_encoding_receipt_hash,
            substrate_contract_hash=receipt.substrate_contract_hash,
            substrate_profile_id=receipt.substrate_profile_id,
            expected_encoding_entry_hashes=receipt.expected_encoding_entry_hashes,
            observed_encoding_entry_hashes=receipt.observed_encoding_entry_hashes,
            drifted_encoding_entry_hashes=receipt.drifted_encoding_entry_hashes,
            missing_encoding_entry_hashes=receipt.missing_encoding_entry_hashes,
            unexpected_encoding_entry_hashes=receipt.unexpected_encoding_entry_hashes,
            encoding_entry_count=receipt.encoding_entry_count,
            drift_count=receipt.drift_count,
            substrate_drift_class=receipt.substrate_drift_class
        )
        if sha256_hex(p) != receipt.substrate_drift_receipt_hash:
            raise ValueError(_ERR_HASH_MISMATCH)
