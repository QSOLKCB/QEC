...
class PanelTopology:
    ...
    def from_dict(cls, payload: object) -> "PanelTopology":
        ...
        banks = _exact_list(item["banks"], label="banks")
        paths = _exact_list(item["paths"], label="paths")
        return cls(
            item["topology_id"],
...
class PanelFaultPlan:
    ...
    def __post_init__(self) -> None:
        for name, values in (("busy_banks", self.busy_banks), ("stalled_motor_groups", self.stalled_motor_groups)):
            for value in values:
    ...
    def from_dict(cls, payload: object) -> "PanelFaultPlan":
        item = _exact_object(
            ...
            fields={"busy_banks", "stalled_motor_groups", "translation_corruption", "sender_disagreement"},
        )
        ...
        return cls(tuple(busy), tuple(stalled), item["translation_corruption"], item["sender_disagreement"])
...
def compile_sender_program(topology: PanelTopology, table: TranslationTable, register: dict[str, object]) -> dict[str, object]:
...
def build_sender_register_receipt(register: dict[str, object], program: dict[str, object]) -> dict[str, object]:
    _request_from_register(register)
    _validate_hashed_artifact(program, schema=SENDER_PROGRAM_SCHEMA, label="sender program")
    if program.get("digit_register_sha256") != register.get("sha256"):
    ...
    def route(self, request: PanelRequest, *, faults: PanelFaultPlan | None = None) -> PanelRouteResult:
        ...
        if fault_plan.translation_corruption:
            ...
            if selected is None:
                ...
                log.append("capacity", "no_admissible_path", {"fallback_rule": fallback_rule})
            else:
...
def validate_route_receipt(receipt: dict[str, object]) -> dict[str, object]:
    ...
    request = _request_from_register(register)
    program = _validate_hashed_artifact(item.get("sender_program"), schema=SENDER_PROGRAM_SCHEMA, label="sender program")
    ...
    if program.get("digit_register_sha256") != register.get("sha256"):  # type: ignore[union-attr]
        raise ValueError("sender programme/register binding mismatch")
    if program.get("topology_sha256") != topology.sha256() or program.get("translation_table_sha256") != table.sha256():
        raise ValueError("sender programme control inputs mismatch")
...
def build_fault_battery(exchange: PanelExchange, request: PanelRequest) -> dict[str, object]:
    ...
    primary_bank = exchange.topology.path(entry.path_candidates[0]).bank
...
def compare_strowger_panel(strowger_receipt: dict[str, object], panel_receipt: dict[str, object]) -> dict[str, object]:
    ...
    unsigned = {
        ...
        "contract_version": "171.2",
        "strowger_receipt_sha256": validate_sha256(strowger_receipt.get("sha256"), "strowger receipt sha256"),
        "panel_receipt_sha256": validate_sha256(panel_receipt.get("sha256"), "panel receipt sha256"),
        "checks": checks,
