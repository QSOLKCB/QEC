...
def _receipt(**kw):
    ...
    h = _hr(m,p)
    c = _cir(m,p,h, kw.pop("citation_passed", True), kw.pop("citation_claim_boundary", "SUPPORTS_METHOD_ONLY"))
...
def test_hash_canonical_validity_recompute_and_idempotent_rebuild():
    ...
    with pytest.raises(ValueError, match="claim_scope_valid mismatch"):
        csr.validate_claim_scope_receipt(forged, m,p,h,c)


def test_enum_validation_malformed_hash_immutable_adapter_and_blockers():
    with pytest.raises(ValueError):
        csr.build_claim_identity("k", "BAD", "s")
    with pytest.raises(ValueError):
        csr.build_claim_evidence_scope("BAD", "r")
    with pytest.raises(ValueError):
        csr.build_claim_support_boundary("BAD", "r")
    with pytest.raises(ValueError):
        csr.build_claim_escalation_boundary("BAD", "r")
    with pytest.raises(ValueError):
        csr.build_claim_uncertainty_declaration("BAD", "r")
    with pytest.raises(ValueError):
        csr.build_claim_benchmark_interpretation("BAD", "r")
    with pytest.raises(ValueError):
        _receipt(review_state="BAD")
    m,p,h,c,r = _receipt()
    with pytest.raises(FrozenInstanceError):
        r.adapter_only = False
    with pytest.raises(ValueError):
        csr.validate_claim_identity(replace(r.claim_identity, claim_identity_hash="abc"))
...
def test_validity_gates_review_uncertainty_support_evidence_adapter_and_citation():
    ...
    with pytest.raises(ValueError, match="adapter_only must be True"):
        _receipt(adapter_only=False)
...
def test_upstream_validation_and_child_validation_before_aggregate():
    m,p,h,c,r = _receipt()
    with pytest.raises(ValueError):
        csr.validate_claim_scope_receipt(r, replace(m, research_automation_manifest_hash="0"*64), p,h,c)
    with pytest.raises(ValueError):
        csr.validate_claim_scope_receipt(r, m, replace(p, paper_generation_provenance_receipt_hash="0"*64), h,c)
    with pytest.raises(ValueError):
        csr.validate_claim_scope_receipt(r, m,p, replace(h, human_review_boundary_receipt_hash="0"*64), c)
    with pytest.raises(ValueError):
        csr.validate_claim_scope_receipt(r, m,p,h, replace(c, citation_integrity_receipt_hash="0"*64))
    forged = replace(r.claim_identity, claim_summary="mutated")
    with pytest.raises(ValueError):
        csr.build_claim_scope_receipt(manifest=m, paper_generation_provenance_receipt=p, human_review_boundary_receipt=h, citation_integrity_receipt=c, claim_identity=forged, evidence_scope=r.evidence_scope, support_boundary=r.support_boundary, escalation_boundary=r.escalation_boundary, uncertainty_declaration=r.uncertainty_declaration, benchmark_interpretation=r.benchmark_interpretation, claim_review_state=r.claim_review_state)


def test_symbolic_empirical_benchmark_hardware_and_forbidden_semantics():
    with pytest.raises(ValueError, match="non-empirical evidence scope"):
        _receipt(evidence="SYMBOLIC_ONLY", category="EMPIRICAL_OBSERVATION")
    with pytest.raises(ValueError, match="non-empirical evidence scope"):
        _receipt(evidence="NON_EMPIRICAL_ONLY", category="EMPIRICAL_OBSERVATION")
    with pytest.raises(ValueError, match="manifest claim-scope declaration"):
        _receipt(category="EMPIRICAL_OBSERVATION", manifest_claim_scope="SYMBOLIC_ONLY")
    with pytest.raises(ValueError, match="symbolic-only inheritance|paper claim inheritance"):
        _receipt(category="EMPIRICAL_OBSERVATION", claim_inheritance="SYMBOLIC_ONLY_INHERITANCE")
    with pytest.raises(ValueError, match="citation claim-boundary mode"):
        _receipt(category="EMPIRICAL_OBSERVATION", citation_claim_boundary="SUPPORTS_BACKGROUND_ONLY")
    with pytest.raises(ValueError, match="benchmark-context-only"):
        _receipt(benchmark="BENCHMARK_CONTEXT_ONLY", category="EMPIRICAL_OBSERVATION")
    with pytest.raises(ValueError, match="reproducibility-not-established"):
        _receipt(category="REPRODUCIBILITY_STATEMENT", benchmark="REPRODUCIBILITY_NOT_ESTABLISHED")
    ...
    with pytest.raises(ValueError):
        csr.build_claim_support_boundary("SUPPORTS_CONTEXT_ONLY", "scientifically proven")
    with pytest.raises(ValueError):
        csr.build_claim_identity("k", "IMPLEMENTATION_DESCRIPTION", "citation proves")
    with pytest.raises(ValueError):
        csr.build_claim_uncertainty_declaration("UNCERTAINTY_DECLARED", "causal certainty")
