...
from qec.analysis.game_world_adapter_contract import (
    build_world_adapter_contract_receipt,
)
...
from qec.analysis.game_world_observation_snapshot import (
    ...
    build_observation_snapshot_set,
    get_allowed_observation_channel_types,
    validate_observation_snapshot,
    validate_observation_snapshot_receipt_with_adapter,
    validate_observation_snapshot_set,
...
def test_channel_spec_basics():
    ...
    assert (
        s.observation_channel_hash
        == build_observation_channel_spec(
            "TEXT_EVENT", "EVENTS", 1024
    ...
    with pytest.raises(ValueError, match="INVALID_CHANNEL_TYPE"):
        build_observation_channel_spec("BAD")
    with pytest.raises(ValueError, match="INVALID_INPUT"):
        build_observation_channel_spec("TEXT_EVENT", "bad")
    with pytest.raises(ValueError, match="INVALID_HASH_FORMAT"):
        replace(s, observation_channel_hash="A" * 64)
...
def test_snapshot_canonical_and_tamper(adapter_ctx):
    ...
    with pytest.raises(ValueError, match="INVALID_OBSERVATION_PAYLOAD"):
        replace(
            a,
            canonical_observation_payload="{not-json",
            observation_payload_hash=sha256_hex("{not-json"),
            observation_snapshot_hash=sha256_hex(
                gwos._observation_snapshot_payload(
                    a.adapter_contract_receipt_hash,
                    a.adapter_spec_hash,
                    a.observation_index,
                    a.observation_channel,
                    "{not-json",
                    sha256_hex("{not-json"),
                )
            ),
...
def test_payload_channel_edges(adapter_ctx):
    c, s = adapter_ctx
    assert build_observation_snapshot(
        c, s, build_observation_channel_spec("PIXEL_HASH"), 0, "a" * 64
    )
    with pytest.raises(ValueError, match="INVALID_HASH_FORMAT"):
        build_observation_snapshot(
            c, s, build_observation_channel_spec("PIXEL_HASH"), 0, "A" * 64
        )
    t = build_observation_snapshot(
        c, s, build_observation_channel_spec("TEXT_EVENT"), 0, "  Hi  "
    )
    ...
    with pytest.raises(ValueError, match="PAYLOAD_TOO_LARGE"):
        build_observation_snapshot(
            c,
            s,
            build_observation_channel_spec("TEXT_EVENT", max_payload_bytes=4),
            0,
            "abcdef",
        )
    with pytest.raises(ValueError, match="INVALID_OBSERVATION_PAYLOAD"):
        build_observation_snapshot(
            c, s, build_observation_channel_spec("SCORE_VALUE"), 0, True
        )
    with pytest.raises(ValueError, match="INVALID_OBSERVATION_PAYLOAD"):
        build_observation_snapshot(
            c, s, build_observation_channel_spec("SCORE_VALUE"), 0, 1.0
        )
    with pytest.raises(ValueError, match="INVALID_OBSERVATION_PAYLOAD"):
        build_observation_snapshot(
            c, s, build_observation_channel_spec("POSITION_VECTOR"), 0, (1, 2)
        )
    with pytest.raises(ValueError, match="INVALID_OBSERVATION_PAYLOAD"):
        build_observation_snapshot(
            c, s, build_observation_channel_spec("POSITION_VECTOR"), 0, [1, True]
        )
    with pytest.raises(ValueError, match="INVALID_OBSERVATION_PAYLOAD"):
        build_observation_snapshot(
            c, s, build_observation_channel_spec("POSITION_VECTOR"), 0, [1, 1.2]
        )
...
def test_action_mask_and_adapter_validators(adapter_ctx, tmp_path):
    ...
    with pytest.raises(ValueError, match="ADAPTER_SPEC_NOT_IN_CONTRACT"):
        validate_spec_in_contract(c2, s)
    if len(codes) > 0:
        bad = ["NOPE"]
        cp = canonical_json(bad)
        ph = sha256_hex(cp)
        s_bad = ObservationSnapshot(
            c.adapter_contract_receipt_hash,
            s.adapter_spec_hash,
            0,
            ch,
            cp,
            ...
            sha256_hex(
                gwos._observation_snapshot_payload(
                    c.adapter_contract_receipt_hash, s.adapter_spec_hash, 0, ch, cp, ph
                )
            ),
        )
        assert validate_observation_snapshot(s_bad)
        with pytest.raises(ValueError, match="ACTION_MASK_UNKNOWN_ACTION"):
            validate_observation_snapshot_with_adapter(s_bad, c, s)
    if len(codes) > 1:
        oo = list(reversed(codes))
        cp = canonical_json(oo)
        ph = sha256_hex(cp)
        s_oo = ObservationSnapshot(
            c.adapter_contract_receipt_hash,
            s.adapter_spec_hash,
            0,
            ch,
        ...
        with pytest.raises(ValueError, match="ACTION_MASK_ORDER_MISMATCH"):
            validate_observation_snapshot_with_adapter(s_oo, c, s)
...
def test_tuple_rejection_and_set_rules(adapter_ctx, monkeypatch):
    ...
    with pytest.raises(ValueError, match="INVALID_OBSERVATION_PAYLOAD"):
        build_observation_snapshot(
            c, s, build_observation_channel_spec("SYMBOLIC_STATE"), 0, {"x": (1, 2)}
        )
    with pytest.raises(ValueError, match="INVALID_OBSERVATION_PAYLOAD"):
        build_observation_snapshot(
            c, s, build_observation_channel_spec("UNKNOWN"), 0, (1, 2)
        )
    with pytest.raises(ValueError, match="INVALID_OBSERVATION_PAYLOAD"):
        build_observation_snapshot(
            c, s, build_observation_channel_spec("ACTION_MASK"), 0, ("A",)
        )
    ...
    with pytest.raises(ValueError, match="INVALID_INPUT"):
        ObservationSnapshotSet(
            ...
            2,
            sha256_hex(
                gwos._observation_snapshot_set_payload(
                    c.adapter_contract_receipt_hash, s.adapter_spec_hash, (r1, r0), 2
                )
            ),
        )
    with pytest.raises(ValueError, match="DUPLICATE_OBSERVATION"):
        build_observation_snapshot_set(
            c, s, [r0, build_observation_snapshot_receipt(c, s, t, 0, "z")]
        )
    monkeypatch.setattr(
        "qec.analysis.game_world_observation_snapshot._MAX_OBSERVATION_SNAPSHOTS", 1
    )
    with pytest.raises(ValueError, match="INVALID_INPUT"):
        build_observation_snapshot_set(c, s, [r0, r1])
...
def test_receipt_set_and_binding_tamper(adapter_ctx):
    ...
    with pytest.raises(ValueError, match="INVALID_HASH_FORMAT"):
        replace(st, observation_snapshot_set_hash="SHORT")
    with pytest.raises(ValueError, match="HASH_MISMATCH"):
        replace(st, observation_snapshot_set_hash="c" * 64)
    bad_snap = replace(
        r.observation_snapshot,
        adapter_contract_receipt_hash="d" * 64,
        observation_snapshot_hash=sha256_hex(
            gwos._observation_snapshot_payload(
                "d" * 64,
                r.observation_snapshot.adapter_spec_hash,
                r.observation_snapshot.observation_index,
                r.observation_snapshot.observation_channel,
                r.observation_snapshot.canonical_observation_payload,
    ...
    with pytest.raises(ValueError, match="SNAPSHOT_ADAPTER_MISMATCH"):
        replace(
            r,
            observation_snapshot=bad_snap,
            observation_snapshot_receipt_hash=sha256_hex(
                gwos._observation_snapshot_receipt_payload(
                    r.adapter_contract_receipt_hash, r.adapter_spec_hash, bad_snap
                )
            ),
        )
...
def test_scope_boundary_scan():
    source_path = (
        ...
        / "qec"
        / "analysis"
        / "game_world_observation_snapshot.py"
    )
